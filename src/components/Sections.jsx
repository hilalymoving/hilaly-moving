...
export function SliderSection({ t }) {
  ...
  const next = () => {
    setSlideIdx((prev) => (prev + 1) % t.slider.items.length)
  }
  const prev = () => {
    setSlideIdx((prev) => (prev - 1 + t.slider.items.length) % t.slider.items.length)
  }
  ...
  useEffect(() => {
    ...
    return () => clearInterval(id)
  }, [t.slider.items.length])

  const items = t.slider.items

  return (
    <section id="slider" style={{ padding: 'clamp(2rem,6vw,4rem) 1rem', position: 'relative', overflow: 'hidden', background: th.bg }}>
      <div style={{ maxWidth: 1100, margin: '0 auto', position: 'relative' }}>
        <SectionHeader title={t.slider.title} sub={t.slider.sub} />

        {/* Slider container */}
        <div style={{ position: 'relative', height: 400, overflow: 'hidden', borderRadius: 20, marginTop: 24 }}>
          {/* Slides */}
          <div style={{
            display: 'flex',
          ...
          {items.map((_, i) => (
            <div
              ...
              style={{
                ...
                height: 10,
                borderRadius: '50%',
                background: slideIdx === i ? '#D4A017' : 'rgba(212,160,23,0.3)',
